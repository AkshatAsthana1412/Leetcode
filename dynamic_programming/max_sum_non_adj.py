...
def max_sum_of_non_adjacent(arr, indx):
    if not arr:
        return None
    if indx == 0:
    ...
    if indx < 0:
        ...
        return 0
    max_sum_with_pick = arr[indx] + max_sum_of_non_adjacent(arr, indx - 2)
    max_sum_without_pick = max_sum_of_non_adjacent(arr, indx - 1)
    return max(max_sum_with_pick, max_sum_without_pick)

if __name__ == '__main__':
    arr = list(map(int, input("Enter list elements: ").split()))
    n = len(arr)
    print(max_sum_of_non_adjacent(arr, n-1))